...
fn main() {
  let system_time_timestamp: u64 = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
  ...
  dbg!(NaiveDateTime::from_timestamp(system_time_timestamp as i64, 0));
  ...
  println!("chrono::Local::now().naive_utc().timestamp()  = {:?}", chrono_naive_utc);
  println!("chrono::Local::now().naive_local().timestamp()  = {:?}", chrono_naive_local);
}